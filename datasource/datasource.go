...
func (r *ReplayNavigationDataProvider) Next() (NavigationDataPoint, bool) {
	...
	ok = r.assignFieldValue(record, fHeading, &result.Heading) &&
		r.assignFieldValue(record, fSpeed, &result.SpeedThroughWater) &&
		r.assignFieldValue(record, fTruewindAngle, &result.TrueWindAngle) &&
		r.assignFieldValue(record, fCumulativeDistance, &result.CumulativeDistance)
